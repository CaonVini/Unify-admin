...
const text = (min: number, max: number) =>
  z.preprocess(clean, z.string().min(min).max(max));
const httpsUrl = z
  .string()
  .url()
...
export const loginSchema = z.object({
  email: z.preprocess(
    clean,
    z.string().email("Informe um e-mail válido.").max(254),
  ),
  password: z
    .string()
...
export const propertySchema = z.object({
  ...
  vacancies: z.number().int().min(1).max(50),
  furnished: z.boolean(),
  petsAllowed: z.boolean(),
  available: z.boolean(),
  photos: z.array(httpsUrl).min(1).max(20),
  amenities: z.array(text(1, 80)).max(30),
  rules: z.array(text(1, 160)).max(30),
  type: z.enum([
    "apartment",
    "house",
    "studio",
...
export const universitySchema = z.object({
  ...
  name: text(1, 160),
  acronym: z.preprocess(clean, z.string().max(30)).optional(),
});

export const catalogNameSchema = z.object({ name: text(1, 80) });
